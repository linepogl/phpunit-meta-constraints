...
class IteratesLikeTest extends TestCase
{
    ...
    public function test_iterates_like(iterable $expected, mixed $actual, ?string $error = null, ?string $expectedAsString = null, ?string $actualAsString = null): void
    {
        ...
        if (null === $error) {
            ...
            self::assertThrows(
                Util::expectationFailure('Custom message', $expected, $actual, $expectedAsString, $actualAsString, $error),
                static fn() => self::assertIteratesLike($expected, $actual, false, 'Custom message'),
            );
            self::assertDoesNotThrow(
                Throwable::class,
                static fn() => self::assertDoesNotIterateLike($expected, $actual, false),
            );
        }
    }
    ...
    public function test_iterates_like_rewind(): void
    {
        ...
        self::assertDoesNotThrow(Throwable::class, static fn() => self::assertIteratesLike([1, 2], new ArrayIterator([1, 2]), rewind: true));
        self::assertThrows(
            new Exception('Cannot traverse an already closed generator'),
            static fn() => self::assertIteratesLike([1, 2], (static function () {
                yield 1;
                yield 2;
            })(), rewind: true),
        );
    ...
    public function test_to_string(): void
    {
        ...
        self::assertIs('does not iterate like an array', self::doesNotIterateLike([1, 2])->toString());
        self::assertIs('does not iterate like an array or rewinds', self::doesNotIterateLike([1, 2], rewind: true)->toString());

        self::assertIs('iterates like an array and iterates like an array', self::logicalAnd(self::iteratesLike([1, 2]), self::iteratesLike([1, 2]))->toString());
    }
}
